shinyServer(function(input, output, session) {
  ...
  # Highest Home Value Index by City Boxc
  ...
  output$levelQueryUi <- renderUI({
    radioButtons("analysisLevel", label = "Cấp độ khu vực",
  ...
  output$stateQuery2Ui <- renderUI({
    ...
    selectInput("state2", label = "Tiểu bang:", choices = c(Choose='', as.character(states)), selected = dflt$state, selectize = FALSE)
  })

  # State query UI
  output$stateQuery3Ui <- renderUI({
    states <- sort(unique(geo$StateName))
    selectInput("state3", label = "Tiểu bang:", choices = c(Choose='', as.character(states)), selected = dflt$state, selectize = FALSE)
  })

  # State query UI
  output$stateQuery4Ui <- renderUI({
    states <- sort(unique(geo$StateName))
    selectInput("state4", label = "Tiểu bang:", choices = c(Choose='', as.character(states)), selected = dflt$state, selectize = FALSE)
  })
  ...
  output$countyQuery3Ui <- renderUI({
    ...
    selectInput("county3", label = "Thị trấn", choices = c(Choose='', as.character(counties$County)), selected = dflt$county, selectize = FALSE)
  })
  ...
  output$countyQuery4Ui <- renderUI({
    ...
    counties <- arrange(unique(subset(geo, StateName == state, select = County)), County)
    selectInput("county4", label = "Thị trấn:", choices = c(Choose='', as.character(counties$County)), selected = dflt$county, selectize = FALSE)
  })
  ...
  output$cityQuery4Ui <- renderUI({
    ...
    selectInput("city4", label = "Thành phố:", choices = c(Choose='', as.character(cities$City)), selected = dflt$city, selectize = FALSE)
  })
  ...
  output$valueHist <- renderPlot({
    withProgress(message = "Rendering Value Histogram", {
      ...
      hist(as.numeric(d$Value), breaks = bins, col = "skyblue", border = "white",
           xlab = "Giá trị du lịch trung bình ($000)", 
           main = "Biểu đồ giá trị du lịch trung bình")
    })
  })
  ...
  output$stateUi <- renderUI({
    states <- sort(unique(geo$StateName))
    selectInput("state", label = "Tiểu bang:", choices = c(Choose='', as.character(states)), selected = dflt$state, selectize = FALSE)
  })
  ...
  output$countyUi <- renderUI({
    ...
    selectInput("county", label = "Thị trấn:", choices = c(Choose='', as.character(counties)), selected = dflt$county, selectize = FALSE)
  })
  ...
  output$cityUi <- renderUI({
    ...
    selectInput("city", label = "Thành phố:", choices = c(Choose='', as.character(cities)), selected = dflt$city, selectize = FALSE)
  })
  ...
  selectHistoricalData <- reactive({
    ...
    validate(
      need(!is.null(d), "Không có dữ liệu để phân tích. Vui lòng chọn một thị trường và nhấn 'Go' để xử lý phân tích.")
    )
    
    validate(
      need(!(nrow(d) == 0), "Không có dữ liệu để phân tích. Vui lòng chọn một thị trường khác và nhấn 'Go' để xử lý phân tích.")
    )

    validate(
      need(!any(is.na(d)), "Không thể tạo chuỗi thời gian có giá trị NA. Vui lòng chọn một thị trường khác trong thanh bên.")
    )
  ...
  output$minPredictionBox <- renderValueBox({
    ...
    valueBox(
      paste0("$", round(min(d$Prediction),0)), paste("Dự đoán giá trị nhà trung bình tối thiểu vào cuối giai đoạn dự báo cho ", l," thị trường."), 
  ...
  output$maxPredictionBox <- renderValueBox({
    ...
    valueBox(
      paste0("$", round(max(d$Prediction),0)), paste("Dự đoán giá trị nhà trung bình tối đa vào cuối giai đoạn dự báo cho ", l," thị trường."), 
  ...
  output$meanPredictionBox <- renderValueBox({
    ...
    valueBox(
      paste0("$", round(mean(d$Prediction),0)), paste("Có nghĩa là dự đoán giá trị nhà trung bình vào cuối giai đoạn dự báo cho ", l," thị trường."), 
